...
export const TestimonialsSection: React.FC = () => {
  ...
  const testimonials: Testimonial[] = [
    {
      id: 1,
      console.log('🎬 Injecting Ignite testimonial videos');
      
      // Testimonial video IDs
      const testimonialVideos = [
        '687c9e4e7d725bff283cc099', // Daniel Carter
        '687c9e611dd0489c9a8ab751', // Marcus Reed
        '687c9e49886aa48cc3165615'  // Rick Alvarez
      ];
      
      testimonialVideos.forEach((videoId, index) => {
        setTimeout(() => {
          // Remove existing script if any
          const existingScript = document.getElementById(`scr_ignite_${videoId}`);
          if (existingScript) {
          ...
          script.id = `scr_ignite_${videoId}`;
          script.async = true;
          script.innerHTML = `
            (function() {
              try {
                console.log('🎬 Loading Ignite testimonial video: ${videoId}');
                var s = document.createElement("script");
                s.src = "https://scripts.converteai.net/b792ccfe-b151-4538-84c6-42bb48a19ba4/players/${videoId}/v4/player.js";
                s.async = true;
                s.onload = function() {
                  console.log('✅ Ignite testimonial video loaded: ${videoId}');
                };
                s.onerror = function() {
                  console.error('❌ Failed to load Ignite testimonial video: ${videoId}');
                };
          ...
          console.log('✅ Ignite testimonial VTurb script injected for video:', videoId);
        }, index * 2000); // Stagger injection by 2 seconds each
      });
    }
    ...
    return () => {
      const testimonialVideos = [
        '687c9e4e7d725bff283cc099',
        '687c9e611dd0489c9a8ab751', 
        '687c9e49886aa48cc3165615'
      ];
      
      testimonialVideos.forEach(videoId => {
        const scriptToRemove = document.getElementById(`scr_ignite_${videoId}`);
        if (scriptToRemove) {
